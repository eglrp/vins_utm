...
//            AddLambdatoHessianLM();
            // 第四步，解线性方程
            SolveLinearSystem();
//            cout << "||delta_x|| = " << delta_x_.norm() << endl;
            //
...
//                cout << "[Solve] - delta_x_ = " << delta_x_.transpose() << endl;
                // 在新线性化点 构建 hessian
                MakeHessian();
...
bool Problem::SolveDogleg(int iterations) {
    ...
    while (!stop && (iter<iterations)){
        cout << "iter" << iter << ": chi = " << currentChi_ << endl;
        bool oneStepSuccess = false;
        int false_cnt = 0;
        while(!oneStepSuccess && false_cnt < 10){
            SolveLinearSystemDogleg();
            UpdateStates();
            oneStepSuccess = IsGoodStepInDogleg();
            if (oneStepSuccess){
//                cout << "[SolveDogleg] - delta_x_ = " << delta_x_.transpose() << endl;
                MakeHessian();
                false_cnt = 0;
            }else{
                false_cnt++;
...
#pragma omp declare reduction (eigenPlus: Eigen::Matrix<double,-1,-1>: omp_out = omp_out+omp_in) \
initializer(omp_priv=Eigen::Matrix<double,-1,-1>::Zero(omp_orig.rows(), omp_orig.cols()))
#pragma omp declare reduction (eigenPlus: Eigen::Matrix<double,-1,1>: omp_out = omp_out+omp_in) \
initializer(omp_priv=Eigen::Matrix<double,-1,1>::Zero(omp_orig.rows(), 1))
...
#pragma omp parallel for reduction(eigenPlus: H, b)
//    for (auto &edge: edges_) {
    for(size_t bkt = 0; bkt<edges_.bucket_count(); ++bkt){
        for(auto edge = edges_.begin(bkt); edge != edges_.end(bkt); edge++) {
...
//    cout << "[SolveLinearSystem] - delta_x_ = " << delta_x_.transpose() << endl;

}

void Problem::SolveLinearSystemDogleg() {
    VecX pB = VecX::Zero(ordering_generic_), pU = VecX::Zero(ordering_generic_);

    VecX  gk = -b_;
    MatXX Bk = Hessian_;

//    delta_x_B = -Bk.inverse() * gk;
    if (problemType_ == ProblemType::GENERIC_PROBLEM){
        pB = Bk.ldlt().solve(-gk);
    }else{ // problem type is SLAM_PROBLEM
        // step1 : schur marginalization --> Bk_pp, gk_pp
        int reserve_size = ordering_poses_;
        int marg_size = ordering_landmarks_;
        MatXX Bk_mm = Bk.block(reserve_size, reserve_size, marg_size, marg_size);
        MatXX Bk_pm = Bk.block(0, reserve_size, reserve_size, marg_size);
        MatXX Bk_mp = Bk.block(reserve_size, 0, marg_size, reserve_size);
        // mgk stands for minus gk, as the equation is Bk * delta_x_B = -gk
        VecX mgk_pp = -gk.segment(0, reserve_size);
        VecX mgk_mm = -gk.segment(reserve_size, marg_size);

        MatXX Bk_mm_inv(MatXX::Zero(marg_size, marg_size));
        // TODO:: use openMP
        for (const auto& landmarkVertex: idx_landmark_vertices_){
            int  idx = landmarkVertex.second->OrderingId() - reserve_size;
            int size = landmarkVertex.second->LocalDimension();
            Bk_mm_inv.block(idx, idx, size, size) = Bk_mm.block(idx, idx, size, size).inverse();
        }

        MatXX tempBk = Bk_pm * Bk_mm_inv;
        MatXX Bk_pp_schur = Bk.block(0,0, ordering_poses_, ordering_poses_) - tempBk*Bk_mp;
        MatXX mgk_pp_schur = mgk_pp - tempBk * mgk_mm;

        // step2: solve Bk_pp * delta_x = mgk_pp
        VecX delta_x_pp(VecX::Zero(reserve_size));

        delta_x_pp = Bk_pp_schur.ldlt().solve(mgk_pp_schur);
        pB.head(reserve_size) = delta_x_pp;

        // step3: solve Bk_mm * delta_x = mgk_mm - Bk_mp * delta_x_pp
        VecX delta_x_ll(marg_size);
        delta_x_ll = Bk_mm_inv * (mgk_mm - Bk_mp * delta_x_pp);
        pB.tail(marg_size) = delta_x_ll;
    }

//     delta_x_P
    pU = - gk.squaredNorm() / (gk.transpose()*Bk*gk) * gk;
//    cout << "[SolveLinearSystemDogleg]\npB = " <<  pB.transpose() << ", pU = " << pU.transpose() << endl;
//    determine tau
    if ( pB.norm() <= trust_region){
        tau_dogleg = 2;
    } else if(pU.norm() >= trust_region){
        tau_dogleg = trust_region/pU.norm();
    }else{
        VecX pB_pU = pB - pU; // vector of pB-pU
        tau_dogleg = (-pU.dot(pB_pU) + sqrt( pow(pU.transpose()*pB_pU,2) - pB_pU.squaredNorm()*(pU.squaredNorm()-trust_region*trust_region) ) ) / (pB_pU.squaredNorm());
        ++tau_dogleg;
    }
//    cout << "[SolveLinearSystemDogleg] - tau = " <<  tau_dogleg << endl;
    assert(tau_dogleg>=0 && tau_dogleg<=2);
    if (tau_dogleg<=1){
        delta_x_ = tau_dogleg * pU;
    }else {
        delta_x_ = pU + (tau_dogleg - 1) * (pB - pU);
    }
//    cout << "[SolveLinearSystemDogleg] - delta_x_ = " <<  delta_x_.transpose() << endl;
}
...
/*double Problem::ComputeAlpha(){
    ...
    for (auto edge: edges_) {
        ...
        assert(jacobians.size() == verticies.size());
        for (size_t i=0; i<verticies.size(); ++i) {
            if (verticies[i]->IsFixed()) continue;
            auto v_i = verticies[i];
            auto jacobian_i = jacobians[i];
            ulong index_i = v_i->OrderingId();
            ulong dim_i = v_i->LocalDimension();
            ...
            MatXX JtW_element = jacobian_i.transpose() * robustInfo;
            cout << "(" << JtW_element.rows() << ", " << JtW_element.cols() << ") ";
...
void Problem::ComputeDoglegInit() {
    trust_region = 1.0;
    TR_MAX = 100;
    eta1_dogleg = 0.15;
    eta2_dogleg = 0.25;
...
bool Problem::IsGoodStepInDogleg() {
    ...
    MatXX Bk = Hessian_;
    double scale;
    scale = -gk.dot(delta_x_) - 0.5 * delta_x_.transpose() * Bk * delta_x_;
    scale += 1e-6; // make sure the denominator is not zero
    double tempChi = 0.0;
    for(auto edge:edges_){
        edge.second->ComputeResidual();
        tempChi += edge.second->RobustChi2();
    ...
    double rho = (currentChi_ - tempChi) / scale;
//    cout << "[IsGoodStepInDogleg] scale = " << scale << ", rho = " << rho << endl;
    // update trust_region first
    if (rho < eta2_dogleg){
        trust_region *= 0.25;
    }else if ( rho > eta3_dogleg && abs(delta_x_.norm() - trust_region)<= 1e-6){
        trust_region = max(trust_region*2, TR_MAX);
    } // otherwise keep the value

    // compare rho with eta1 to determine is it a good step
    if (rho > eta1_dogleg){
        currentChi_ = tempChi;
        return true;
    }else {
